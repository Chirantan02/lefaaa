...
def bbox_expand(img_height, img_width, bbox, exp_ratio):
    ...
    # Fixed expansion ratios with vertical bias and minimum size
    min_size = min(img_width, img_height) * 0.1
    exp_width = max(width * exp_ratio, min_size)
    exp_height = max(height * (exp_ratio + 0.2), min_size)  # Increased vertical bias
    
    # Calculate expanded coordinates from center with boundary checks
    new_x_min = max(0, center_x - (exp_width / 2))
    new_y_min = max(0, center_y - (exp_height / 2))
    new_x_max = min(img_width - 1, center_x + (exp_width / 2))
    new_y_max = min(img_height - 1, center_y + (exp_height / 2))
    
    return int(np.round(new_x_min)), int(np.round(new_y_min)), int(np.round(new_x_max)), int(np.round(new_y_max))
...
def get_arguments():
    ...
    parser.add_argument("--exp_ratio", type=float, default=1.1)
