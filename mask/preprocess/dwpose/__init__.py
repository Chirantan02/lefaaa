    ...
    def __init__(self, model_root, device):
        ...
        try:
            ...
            if 'bad allocation' in str(e):
                print("\nWARNING: Memory allocation failed. Attempting with reduced memory settings...")
                # Try again with reduced memory settings
                import gc
                gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                self.pose_estimation = Wholebody(model_root, 'cpu')
            else:
                raise e
